...
SELECT
  COUNT(DISTINCT(produktname)) AS "Anzahl"
FROM
  produkt;
...
FROM
  ...
  INNER JOIN (
    ...
    HAVING
      COUNT(*) > 1
  ) as dup ON p.produktname = dup.produktname
  AND p.farbe = dup.farbe
  AND p.felgen_in_zoll = dup.felgen_in_zoll
  AND p.innenausstattung = dup.innenausstattung;
